...
// Create a t9 folder to store outputs if it doesn't exist 
capture mkdir "tables/t9/"

/**********************************************************************************
Followed recommendations (Basal) 
**********************************************************************************/
...
esttab using "tables/t9/followed_rec_basal.tex", replace se noobs frag not label tex star(* 0.10 ** 0.05 *** 0.01) ///
...
/**********************************************************************************
Amount of fertilizer applied (kg/ha), basal
**********************************************************************************/
eststo clear
...
esttab using "tables/t9/fert_applied_basal.tex", replace se noobs not label tex star(* 0.10 ** 0.05 *** 0.01) ///
...
esttab using "tables/t9/gap_basal.tex", replace se noobs not label tex star(* 0.10 ** 0.05 *** 0.01) ///
...
********************************************************************************************************
* Extensive margin consistent with recommendation (full season)
********************************************************************************************************
...
if "`x'" == "urea" {
	generate followed_rec_`x' = used_`x'
}
else{
	generate followed_rec_`x' = used_`x'
	replace followed_rec_`x' = 1 - used_`x' if irrigation_ml == 0 //Unirrigated plots should not have MOP, Zinc or DAP applied
}

egen std_`x' = std(followed_rec_`x')

replace followed_rec_`x' = . if missing(used_`x')

summarize followed_rec_`x' if treatment == 0, meanonly 
local varMean = `r(mean)' 
eststo: regress followed_rec_`x' treatment i.block_id, robust
estadd scalar depMean = `varMean'
} 
...
local var_num = 1
**Defining list of variables looping over, for eventual combination of estimates
foreach x of varlist followed_rec_urea followed_rec_dap followed_rec_mop followed_rec_zinc {
...
esttab using "tables/t9/followed_rec_fullseason.tex", replace se noobs frag not label tex star(* 0.10 ** 0.05 *** 0.01) ///
noomitted nobaselevels scalars("N Observations" "r2_a Adjusted \$R^2$" "depMean Control mean") sfmt(%9.0fc %9.3fc %9.3fc) noconstant ///
indicate("Block FE = *block*") mtitles("\makecell[c]{UREA}" "\makecell[c]{DAP}" "\makecell[c]{MOP}" "\makecell[c]{Zinc}" "\makecell[c]{Standardized \\ joint effects}")
...
**Calculate average standardized effect across outcomes  
...
esttab using "tables/t9/fert_applied_fullseason.tex", replace se noobs not label tex star(* 0.10 ** 0.05 *** 0.01) ///
...
esttab using "tables/t9/gap_fullseason.tex", replace se noobs rename(v1 treatment) r2 frag not label tex star(* 0.10 ** 0.05 *** 0.01) ///
